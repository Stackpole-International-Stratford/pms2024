...
def autopopulate_downtime_event_check_cron(request):
    ...
    MONTH_THRESHOLD = 30 * 24 * 3600  # seconds
    now_ts = time.time()
    recent_threshold_ts = now_ts - DOWN_THRESHOLD * 60
    month_threshold_ts  = now_ts - MONTH_THRESHOLD
    ...
    try:
        ...
        log(f"✅ Connected to GFxPRoduction as {conn.get_host_info()}")
        cursor = conn.cursor()
        ...
        for m in machines:
            ...
            if last_ts is None or last_ts < month_threshold_ts:
                log(f"→ Skipping {m}: no production entries in the last 30 days.")
                continue
        ...
        log(f"🚀 Would create {missing_count} new downtime event(s).")
