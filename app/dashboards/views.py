...
def get_pages_config(name="PAGES"):
    """
    Returns the JSON blob as a native Python dict.
    Falls back to an empty dict if not yet set up.
    """
    try:
        cfg = PagesConfig.objects.get(name=name)
        return cfg.data
    except ObjectDoesNotExist:
        # You can choose to log a warning here, or even raise,
        # but falling back to {} is often safer for dashboards.
        return {}
